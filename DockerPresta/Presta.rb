...
class Presta
    ...
    @api_key = 'UZ7ZKZIJ89WL8N974SX4MS3ZNKBUWBNW'
    @api_url = "http://172.27.0.2/api/" 
    ...
    cart = HTTParty.post("#{@api_url}carts/#{@ws_key}",
    {
        ...
        "<prestashop>
          <cart>
            ...
            <allow_seperated_package>#{allow_seperated_package}</allow_seperated_package>
            <date_add>#{date_add}</date_add>
            <date_upd>#{date_upd}</date_upd>
            <associations>
              <cart_rows>
...
##### CUSTOMER THREADS ################################################################################################################################################################
  ...
  def self.post_order_carriers(
    ...
    def self.update_order_body(id:,current_state:,id_carrier:)
    ...
    body.gsub!(/<id_shop.*/," <id_shop>1</id_shop>")  
    body.gsub!(/<id_shop_group.*/," <id_shop_group>1</id_shop_group>")
  ...
  def self.delete_supply_orders(id:)
    ...
    def self.sending_query(id_carrier:)
      con = Mysql2::Client.new(:host=>"172.27.0.3",:port=>"3307",:username=>"root",:password=>"prestashop",:database=>"prestashop")
      x = 1
      while x < 4
        result = con.query("INSERT INTO prestashop.ps_carrier_group (id_carrier,id_group) VALUES (#{id_carrier},#{x})")
        x = x + 1
      end
      x = 1
      while x < 9
        result = con.query("INSERT INTO prestashop.ps_carrier_zone (id_carrier,id_zone) VALUES (#{id_carrier},#{x});")
        x = x + 1
      end
      con.close
    end
...
#Presta.post_deliveries(id_carrier:94,id_range_price:1,id_range_weight:24,id_zone:1,price:10.0)
#Presta.post_carriers(name:"nowy DPD",is_free:0,delay:"DPD")


Presta.sending_query(id_carrier:6)
# x= 1
# while x < 9
#   Presta.post_deliveries(id_carrier:6,id_range_price:0,id_range_weight:1,id_zone:x,price:25.0)
#   x = x + 1
# end